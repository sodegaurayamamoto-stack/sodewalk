...
class _GauraCollectionPageState extends State<GauraCollectionPage> {
 ...
 Map<String, String> _gauraImageMap = {};
 ...
 Future<void> _loadData() async {
   ...
   final locationsData = json.decode(results[1] as String);
   final manifestMap = json.decode(results[2] as String) as Map<String, dynamic>;

   // idだけ取得
   final locationIds = (locationsData['locations'] as List)
   ...
   for (final path in manifestMap.keys) {
     if (path.startsWith('assets/gaura/') && path.endsWith('.png')) {
           ...
           Padding(
             ...
             child: Row(
               children: [
                 ...
                 const Spacer(),
                 if (!_isLoading)
                   Text('${_collectedIds.length} / ${_locationIds.length}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.orange)),
               ],
             ),
           ),
           Padding(
